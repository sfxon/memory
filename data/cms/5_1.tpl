<div class="fix-button-to-bottom" id="start-intro-page">
		...
		{literal}
				<script>
						$(function(){
								$(".mv-type-text").typed({
										strings: ["{/literal}{$smarty.const.MEMORY_TEXT_7}{literal}"],
										typeSpeed: 0,
										callback: function() {
		...
		<div id="try_again_button">
				<a href="{$SITE_URL}info.html"><button class="mv-btn">{$smarty.const.MEMORY_TEXT_14}</button></a>
				&nbsp;&nbsp;&nbsp;
				<a href="{$SITE_URL}"><button class="mv-btn">{$smarty.const.MEMORY_TEXT_8}</button></a>
		</div>
</div>
