...
class cProductconditions extends cModule {
		///////////////////////////////////////////////////////////////////////////////////////////////////
		...
		public static function loadActive($default_datalanguage = 1) {
				...
				$db->setQuery('SELECT * FROM ' . $db->table('productconditions') . ' WHERE status = 1');
				$result = $db->execute();
				...
				while($result->next()) {
						...
						$db->useInstance('systemdb');
						$db->setQuery(
								'SELECT * FROM ' . $db->table('productconditions_description') . ' ' .
								'WHERE ' .
										'productconditions_id = :productconditions_id AND ' .
										'language_id = :language_id'
						);
						$db->bind(':productconditions_id', (int)$tmp_data['id']);
						...
						if(empty($tmp)) {
								...
								$db = core()->get('db');
								$db->useInstance('systemdb');
								$db->setQuery('SELECT * FROM ' . $db->table('productconditions_description') . ' WHERE productconditions_id = :productconditions_id LIMIT 1');
								$db->bind(':productconditions_id', (int)$tmp_data['id']);
								$result = $db->execute();
		...
		public static function loadById($id) {
				...
				$db = core()->get('db');
				$db->useInstance('systemdb');
				$db->setQuery('SELECT * FROM ' . $db->table('productconditions') . ' WHERE id = :id');
				$db->bind(':id', (int)$id);
				$result = $db->execute();
				...
				foreach($data_langs as $lang) {
						...
						$db->bind(':productconditions_id', (int)$id);
