...
class cWssellerlogin extends cModule {
		...
		public static function setExecutionalHooks() {
				...
				if(isset($_SESSION['seller_id'])) {
						header('Location: //' . cSite::loadSiteUrl(core()->get('site_id')) . 'seller/index.html');
						die;
				}
		...
		public function process() {
				...
				$content = $cms->loadContentDataByKey('WSSELLER_LOGIN');
				...
				$site_url = cSeourls::loadSeourlByQueryString('s=cWssellerlogin');
		...
		public function doLogin($email, $password) {
				...
				if(isset($tmp['id'])) {
						...
						if(true === password_verify($password, $password_hash)) {		//Aaaaaaand - you are logged in!
								...
								$_SESSION['seller_id'] = (int)$tmp['id'];
								header('Location: http://' . cSite::loadSiteUrl(core()->get('site_id')) . 'seller/index.html');
								die;
						}
				}
